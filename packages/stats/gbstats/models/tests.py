...
class BaseABTest(ABC):
    ...
    def compute_moments_result(self) -> EffectMomentsResult:
        ...
        if self.config.post_stratify:
            ...
            # raise ValueError(['jill summable_stats', summable_stats, 'angie stats', [(self.stat_a, self.stat_b)]])
...
class EffectMomentsPostStratification:
    ...
    def compute_result(self) -> EffectMomentsResult:
        ...
        if len(cells_for_analysis) == 1:
            ...
            return EffectMoments([(self.stat_a, self.stat_b)], EffectMomentsConfig(difference_type="relative" if self.relative else "absolute")).compute_result()  # type: ignore
        strata_results = []
